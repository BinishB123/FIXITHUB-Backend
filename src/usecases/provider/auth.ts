...
class ProviderAuthInteractor implements IProviderAuthInteractor {
    ...
    async sendOtp(
        ...
        try {
            ...
            const providerExistResponse =
                await this.providerAuthRepository.providerExist(email);

            if (providerExistResponse) {
                return { created: false, message: "Email already exists." };
            }

            const repositoryResponse = await this.providerAuthRepository.sendOtp(
                mailResponse.otp + "",
                email
            );
    ...
    async verify(
        ...
        try {
            const verifyResponse = await this.providerAuthRepository.verifyOtp(
                email,
                otp
            );
            if (verifyResponse) {
            ...
            return {
                success: false,
                message: "An error occurred verification failed",
            };
        }
    }
    ...
    async registerProvider(
        ...
        try {
            const providerCreate =
                await this.providerAuthRepository.registerProvider(registerdata);
            if (providerCreate.message === "server down") {
                return { created: false, message: providerCreate.message };
            } else if (
                providerCreate.message === "registration failed" &&
                !providerCreate.created
            ) {
                return { created: false, message: providerCreate.message };
            }
            const accessToken = await this.jwtServices.generateToken(
                {
                    id: providerCreate.provider?.id,
            ...
            return {
                created: true,
                message: "created",
                provider: providerCreate.provider,
    ...
    async signInProvider(
        ...
        success: boolean;
        message: string;
        provider?: SignResponse;
        accessToken?: string;
        refreshToken?: string;
    }> {
        try {
            const provider =
                await this.providerAuthRepository.signInProvider(providerSignData);
            if (!provider.success) {
                return { success: false, message: provider.message };
            }
            const acccessToken = await this.jwtServices.generateToken(
                {
