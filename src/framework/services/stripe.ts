...
class StripePayment implements IStripe {
  async userCheckoutSession(initailAmountToPay:number): Promise<{ success?: boolean; sessionId?: string; url?: string; }> {
    try {
  ...
  async refund(payemnt_IntendId: string, amount: number): Promise<{ success?: boolean; }> {
    try {
  ...
  async fullpayment(selectedServices: SelectedService[], docId: string): Promise<{ success?: boolean; url?: string; }> {
    try {
