...
class ChatRepo implements IChatRepo {
  ...
  async updateChats(topassChat: string, whotosendthesechatid: string): Promise<{ success?: boolean; chats?: IChatingUser[]; }> {
    try {
      ...
      const data: IChatingUser[] = await chatModel.aggregate([
        {
          $match: {
            [`${topassChat === "user" ? "userId" : "providerId"}`]: new mongoose.Types.ObjectId(whotosendthesechatid),
          },
        },
        {
          $lookup: {
  ...
  async fetchChats(whom: string, id: string): Promise<{ success?: boolean; chats: IChatingUser[]; }> {
    try {
      ...
      const data: IChatingUser[] = await chatModel.aggregate([
        {
          $match: {
            [`${whom === "user" ? "userId" : "providerId"}`]: new mongoose.Types.ObjectId(id),
          },
        },
        {
          $lookup: {
  ...
  async addNewMessage(sender: string, chatId: string, message: string): Promise<{ success?: boolean; messageCreated: any; }> {
    try {
      ...
      const update = await chatModel.updateOne({ latestMessage: createdMesage._id })
  ...
  async liveMessageSeen(messageId: string): Promise<{ success?: boolean; }> {
    try {
      const updateOne = await messageModel.updateOne({_id:new mongoose.Types.ObjectId(messageId)},{
        $set:{
          seen:true
        }
      })
